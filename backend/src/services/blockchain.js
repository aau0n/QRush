...
/** Hardhat localhost: getTransactionCount can lag; long-lived server nonce drifts. */
let nextNonce = null;
...
exports.useTicketNFT = async (tokenId, nonceField, vcHash, proof) => {
  ...
  const receipt = await sendAndWait((overrides) =>
    ticketNFT.useTicket(tokenId, BigInt(nonceField), toBytes32(vcHash), pA, pB, pC, overrides)
  );
  return { txHash: receipt.hash };
};
